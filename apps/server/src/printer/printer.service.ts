...
export class PrinterService {
  ...
  private readonly disablePrintService: boolean;
  ...
  constructor(
    ...
    // Check for DISABLE_PRINT_SERVICE in the environment variables
    const disablePrintServiceStr = process.env.DISABLE_PRINT_SERVICE || 'false';
    this.disablePrintService = disablePrintServiceStr === 'true';
    
    if (!this.disablePrintService) {
      try {
        ...
        this.ignoreHTTPSErrors = this.configService.get<boolean>("CHROME_IGNORE_HTTPS_ERRORS") || false;
      } catch (error) {
        // If we can't get the Chrome URL or token, disable the print service
        this.disablePrintService = true;
        this.browserURL = '';
        this.ignoreHTTPSErrors = false;
        this.logger.warn('Print service is disabled due to missing Chrome configuration.');
      }
    } else {
      this.browserURL = '';
      this.ignoreHTTPSErrors = false;
      this.logger.warn('Print service is disabled. PDF generation and previews will not work.');
    }
  }
  ...
  private async getBrowser() {
    if (this.disablePrintService) {
      throw new InternalServerErrorException(
        ...
        'Print service is disabled. Please enable it by setting DISABLE_PRINT_SERVICE=false'
      );
    }
  ...
  async printResume(resume: ResumeDto) {
    if (this.disablePrintService) {
      this.logger.warn(`Print service is disabled. Returning placeholder URL for resume #${resume.id}`);
      return `https://example.com/disabled-print-service-${resume.id}.pdf`;
    }
  ...
  async printPreview(resume: ResumeDto) {
    if (this.disablePrintService) {
      this.logger.warn(`Print service is disabled. Returning placeholder URL for preview of resume #${resume.id}`);
      return `https://example.com/disabled-preview-service-${resume.id}.jpg`;
    }
